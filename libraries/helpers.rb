  ...
  module Mysql
    ...
    module Helpers
      ...
      def assign_root_password_cmd
        ...
        str << ' -u root password '
        str << node['smm_mysql']['server_root_password']
      end

      def install_grants_cmd
        str = '/usr/bin/mysql'
        str << ' -u root '
        node['smm_mysql']['server_root_password'].empty? ? str << ' < /etc/mysql_grants.sql' : str << " -p#{node['smm_mysql']['server_root_password']} < /etc/mysql_grants.sql"
      end
    end
  end
end
